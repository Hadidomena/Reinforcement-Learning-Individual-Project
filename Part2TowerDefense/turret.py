...
# New turret type: TurretSlow
class TurretSlow(Turret):
  def __init__(self, sprite_sheets, tile_x, tile_y, shot_fx):
    super().__init__(sprite_sheets, tile_x, tile_y, shot_fx)
    self.slow_amount = 0.5  # slows speed by 50%
    self.slow_duration = 60  # frames or ticks

  def deal_damage(self, enemy):
    # Deal less damage and apply slow
    enemy.health -= int(c.DAMAGE * 0.7)
    if not hasattr(enemy, 'slow_timer') or enemy.slow_timer <= 0:
      enemy.speed *= self.slow_amount
      enemy.slow_timer = self.slow_duration

# New turret type: TurretPowerful
class TurretPowerful(Turret):
  def __init__(self, sprite_sheets, tile_x, tile_y, shot_fx):
    super().__init__(sprite_sheets, tile_x, tile_y, shot_fx)
    # Increase the cooldown by 50%
    self.cooldown = self.cooldown * 1.5
  
  def deal_damage(self, enemy):
    # Deal 2.5x damage but with slower fire rate
    enemy.health -= int(c.DAMAGE * 2.5)